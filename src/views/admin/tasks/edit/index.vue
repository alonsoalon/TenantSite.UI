  ...
  <section>
    ...
    >
      <section class="drawer-body">
        ...
        >
          ...
          <el-form-item label="触发器类型" prop="triggerType">
            <el-radio-group v-model="dataItem.triggerType" @change="onTriggerTypeChange">
              <el-radio-button
          ...
          <el-divider content-position="left"></el-divider>
          <el-tooltip class="item" effect="dark" content=" 任务开始时间未设定，默认从第一次执行任务的时间" placement="top-start">
          ...
          <el-tooltip class="item" effect="dark" content=" 任务结束时间未设定，表示永久有效" placement="top-start">
          ...
          <el-tooltip class="item" effect="dark" content="注意实体名称大小写，与程序保持一致" placement="top-start">
          ...
          <el-form-item label="默认系统库" prop="isDefaultDatabase">
            <el-switch v-model="dataItem.isDefaultDatabase" @change="onDefaultDatabase"></el-switch>
          </el-form-item>
          <template v-if="dataItem.isDefaultDatabase === false">
            <el-form-item label="库类型" prop="dbType"><!--, trigger: 'blur' -->
            ...
            <el-tooltip class="item" effect="dark" content="为确保任务正常执行，请填写正确的连接字符串" placement="top-start">
          ...
          <el-tooltip class="item" effect="dark" content="请填写正确的JSON字符串" placement="top-start">
            <el-form-item label="参数" prop="jobParams">
              <el-input type="textarea" rows="4" v-model="dataItem.jobParams" autocomplete="off" @input="$forceUpdate()" />
            </el-form-item> 
          </el-tooltip>
          <el-form-item label="任务描述" prop="remark">
            <el-input type="textarea" v-model="dataItem.remark" autocomplete="off" @input="$forceUpdate()"  />
          </el-form-item>
      ...
      <div class="drawer-footer">
        <el-button @click.native="drawerVisible = false">{{ this.$t("common.cancel") }}</el-button>
        <confirm-button :validate="formValidate" :loading="loading" type="submit" @click="onSubmit" />
      </div>
    </el-drawer>
  </section>
</template>
...
// apis
import { execUpdate } from "@/api/admin/task-qz";
...
export default {
  ...
  data() {
    return {
      ...
      formRules: {
        ...
        jobGroup: [{ required: true, message: "请输入表达式", trigger: "blur" }],
        className:[{ required: true, message: "请填写任务实体名称", trigger: "blur" }],
        dbType:[{ required: true, message: '请指定数据库类型', trigger: "change"}],
        connectionString:[{ required: true, message: "请填写指定数据库的连接字符串", trigger: "blur" }]
      },
      dialogVisible: false,
      dbTypes: [
  ...
  watch:{
    data(val){
      this.dataItem=val;
      console.log(this.dataItem.cron);
      if(this.dataItem.connectionParam&&!this.dataItem.isDefaultDatabase){
        const _d=JSON.parse(this.dataItem.connectionParam);
        this.$set(this.dataItem,'connectionString',_d.ConnectionString);
        this.$set(this.dataItem,'dbType',_d.DbType)
      }
    }
  }
};
