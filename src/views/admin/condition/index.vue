...
// 接口
import { getList, execSoftDelete } from "@/api/admin/condition";
...
let pageConfig = {
  ...
  pageName: "admin--condition--index"
};
  ...
  methods: {
    ...
    onEdit(index, row) {
      ...
      if (row.condition == null || row.condition == "") {
        row.condition = '[{"root":true,"logic":"And","filters":[]}]';
      }
      row.jsonWhere = JSON.parse(row.condition);
